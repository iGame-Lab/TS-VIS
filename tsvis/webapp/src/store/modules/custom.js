...
const actions = {
  ...
  async getScalarDataInterval(context, param) {
    await http.useGet(port.category.scalar, param).then(res => {
      ...
      context.commit("setScalarDataInterval", [
        param["run"],
        param["tag"],
        res.data.data
      ]);
  ...
  async getAudioDataInterval(context, param) {
    await http
      .useGet(param[1]["content"]["port"], param[1]["content"]["parameter"])
      .then(res => {
        if (+res.data.code !== 200) {
          context.commit(
            "setErrorMessage",
        ...
        context.commit("setAudioDataInterval", [
          param[0],
          param[1],
          res.data.data
        ]);
  ...
  async getImageDataInterval(context, param) {
    await http
      ...
      .then(res => {
        ...
        context.commit("setImageDataInterval", [
          param[0],
          param[1],
          res.data.data
        ]);
  ...
  async getTextDataInterval(context, param) {
    await http
      ...
      .then(res => {
        ...
        context.commit("setTextDataInterval", [
          param[0],
          param[1],
          res.data.data
        ]);
  ...
  async getHistDataInterval(context, param) {
    await http
      .useGet(port.category.histogram, { run: param.run, tag: param.tag })
      .then(res => {
        ...
        context.commit("manageHistData", {
          data: res.data.data[param.tag],
          index: param.index
        });
      });
  },
  async getDistDataInterval(context, param) {
    await http
      .useGet(port.category.distribution, { run: param.run, tag: param.tag })
      .then(res => {
        if (Number(res.data.code) !== 200) {
          context.commit(
            "setErrorMessage",
  ...
  async getStatisticDataInterval(context, param) {
    ...
    if (
      param[1].componentName === "threed" ||
      param[1].componentName === "orthographic"
    ) {
...
const mutations = {
  ...
  setAudioData: (state, param) => {
    const paramStringIndex = `${param.content.run}/${
      Object.keys(param.content.value)[0]
    }`;
    let flag = true;
  ...
  setData: (state, param) => {
    ...
    switch (param) {
      case "scalar":
        Object.assign(
          state.scalarmid,
          JSON.parse(JSON.stringify(state.scalar))
        );
        Object.assign(
          state.scalarData,
          JSON.parse(JSON.stringify(state.scalar))
  ...
  manageDistData: (state, param) => {
    ...
    const newStatisticData = JSON.parse(
      JSON.stringify(state.statisticData[param.index])
    );
  ...
  manageHistData: (state, param) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      onedata[onedata.length - 1][1] =
        onedata[onedata.length - 1][0] + binWidth / 2;
      const newOneData = [];
      let binleft = min;
      ...
      for (let j = 0; j < binNum; j++) {
        ...
        for (; curbucket < onedata.length - 1; curbucket++) {
          ...
          if (binright <= onedata[curbucket][1]) {
            if (curBinWidth !== 0)
              count +=
                ((binright - maxleft) / curBinWidth) * onedata[curbucket][2];
            break;
          } else {
            if (curBinWidth !== 0)
              count +=
                ((onedata[curbucket][1] - maxleft) / curBinWidth) *
                onedata[curbucket][2];
          }
        }
