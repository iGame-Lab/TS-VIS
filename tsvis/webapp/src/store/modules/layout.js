    ...
    await http.useGet(port.manage.initCategory, {}).then(res => {
      ...
      if (categorys.length !== 0) {
        categorys.forEach(ce => {
          ...
          if (
            ce === constants.CATEGORYORDER[categoryOrder[0]] &&
            cate === "index"
          ) {
            context.dispatch(
      ...
      context.commit(
        "setRunCategory",
        constants.CATEGORYORDER[categoryOrder[newIndex]]
      );
    });
  },
  ...
  async timingFeatchCategory(context, parm) {
    ...
    const t = setInterval(async () => {
      http.useGet(port.manage.initCategory, { test: 1 }).then(res => {
        ...
        Object.keys(dataCategoryInfo).forEach(val => {
          categorys = categorys.concat(
            Object.keys(dataCategoryInfo[val]).filter(
              v => !categorys.includes(v)
            )
          );
          runFile.push(val);
        });
        ...
        if (cate === "index") {
          ...
          newIndex = categoryOrder.indexOf(
            constants.CATEGORYORDER.indexOf(cate)
          );
        }
        if (categorys.length !== 0) {
          categorys.forEach(ce => {
