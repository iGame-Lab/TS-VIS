...
<template>
  <div class="layout-root layout-same-style" style="width:100%">
    <el-dialog :visible.sync="dialogFormVisible" title="图片保存选项" class="layout-svg-save-dialog">
      <el-form :model="form">
    ...
    <el-dialog :visible.sync="initWaitVisible" title="全力加载中,请稍等..." class="layout-svg-save-dialog">
    ...
    <el-dialog :visible.sync="operationGuide" title="操作指南" class="layout-svg">
      <div id="write" class="">
        ...
        <h4>
          <a name="4-8维操作" class="md-header-anchor"/>
        <span>4-8维操作</span></h4>
        <ol>
          <li>
          <span>可以对点(线条)进行点击，数据信息栏会显示存储的图片/文本</span></li>
          ...
          <li>
            ...
            <code>概率密度</code>
            <span>窗口</span>
          <span>勾选对应的标签会显示对应标签的统计数据，最多勾选两个</span></li>
          <li>
          <span>可以在轴上拖拽出一个灰色矩形框，然后对局部进行高亮</span></li>
        </ol>
        <h3>
    ...
    <el-dialog :visible.sync="dataSyncVisible" title="设定数据同步间隔" class="layout-svg-save-dialog">
      <div>
        <el-checkbox v-model="syncDataBool">启用同步</el-checkbox>
      </div>
      <div style="margin-top: 10px">
        <el-input-number v-model="timeSync" :min="30" :disabled="!syncDataBool" class="input-number" size="mini" />秒
      </div>
    </el-dialog>
    <div ref="left" :style="{height: leftStyle.height, width: leftStyle.width}" class="layout-sidebar layout-same-style">
      <!-- <div ref="left" class="layout-sidebar layout-same-style" style="width:137px"> -->
      <div class="layout-logo layout-same-style">
        <img :class="[logoImgStyleFlage?'img-after':'']" src="static/img/left_logo.png" alt="">
        <!-- <img v-show="logoFlag" :class="[logoImgStyleFlage?'img-after':'']" :src="titleSrc" alt="" style="margin-left:5%;height:10px;width:50px"> -->
        <!-- <span v-show="logoFlag" :style="{fontSize: autoFontSize + 'px'}">天枢</span> -->
      </div>
      <div :class="['layout-sidebar-category-container']">
        <!-- <div class="layout-sidebar-category layout-same-style" :style="{height: allCategoryInform.length * 7 + '%'}"> -->
        <div class="layout-sidebar-category layout-same-style">
          <div
            v-for="(item, i) in allCategoryInform"
            :key="item.id"
            :class="['layout-sidebar-category-each']"
            :style="{height: 100/allCategoryInform.length + '%', lineHeight: 100/allCategoryInform.length + '%'}"
          >
            <!-- <div ref="eachTop" :class="['layout-sidebar-category-each-top']" :style="[{borderRadius:(item.id==selected||i==initId && initFlag?actegoryClickStyle.divTop:'0')}]" /> -->
            <div ref="eachTop" :class="['layout-sidebar-category-each-top']" />
            <div ref="eachMiddle" :class="['layout-sidebar-category-each-middle']" :style="item.id==selected||i==initId && initFlag?{borderLeft:'4px solid #2e4fde'}:{}">
              <!-- :style="[{borderRadius:(item.id==selected||i==initId && initFlag?actegoryClickStyle.divAim:'0')}]" -->
              <router-link
                :to="item.routerName"
                :class="[item.id==selected||i==initId && initFlag?&quot;category-selected&quot;:&quot;&quot;]"
                tag="li"
                @click.native="change(item)"
              >
                <!-- <i :class="['iconfont', item.icon]" :style="{fontSize: autoFontSize + 2 + 'px', marginRight: autoFontSize - 4 + 'px'}" /> -->
                <i :class="['iconfont', item.icon]" :style="{fontSize: 11+ 'px'}" />
                <span :class="[leftStyle.categoryFlage?'span-after':'']" :style="item.id==selected||i==initId && initFlag?{fontSize: 11 + 'px' , color: 'rgb(46, 79, 222)' }:{fontSize: 11 + 'px'}">{{ item.name }}</span>
              </router-link>
            </div>
            <!-- <div :class="['layout-sidebar-category-each-bottom']" :style="[{borderRadius:(item.id==selected||i==initId && initFlag?actegoryClickStyle.divBottom:'0')}]" /> -->
    ...
    <div :style="{height: rightStyle.height, width: rightStyle.width}" class="layout-header-content layout-same-style">
      <div class="layout-header layout-same-style">
        <div :class="['same-div']" style="width:400px">
          ...
          <p :class="['tool-p']" />
          <!-- <p :class="['run-select-container']">
            <i class="iconfont icon-wenjianjia1" style="font-size: 18px; color:whitesmoke;" />
          </p> -->
        ...
        <div :class="['same-div', 'search-tag']" style="width:400px">
          <p
            :class="['run-select-container']"
            :style="{margin: '0 0 0 0'}"
          >
            <i :class="['el-icon-refresh']" style="font-size: 19px; color:whitesmoke;" @click="isClicked" />
            <i
          ...
          <p :class="['tool-p']" :style="{marginRight: '1%'}" />
          <p
            :class="['run-select-container']"
            :style="{margin: '0 2% 0 0'}"
          >
            <i class="iconfont icon-ziyuan108" style="font-size: 19px; color:whitesmoke;" />
          </p>
          <p :class="['run-select-container']" :style="{width: '35%'}">
            <input v-model="inputValue" :class="['tag-search']" placeholder="Enter a Tag">
          </p>
        </div>
      </div>
      <div id="full-screen1" class="layout-content layout-same-style" style="background-color:white">
        <div
        ...
        <router-view id="full-screen" :class="[rawShape, 'layout-same-style']" :style="{width: contentsStyle.leftWidth}" />
        <router-view v-show="contentsStyle.rightShowFlag" name="right" class="layout-content-paramenter layout-same-style" />
      </div>
    </div>
  </div>
</template>
...
const { mapMutations: mapLayoutMutations, mapActions: mapLayoutActions, mapGetters: mapLayoutGetters, mapState: mapLayoutState } = createNamespacedHelpers('layout')
const {
...
export default {
  ...
  mounted() {
    ...
    this.selectedRaw = this.$route.path.split('/')[2]
  ...
  methods: {
    ...
    isClicked() {
      d3.select('.el-icon-refresh')
        .attr('class', 'el-icon-refresh anim')
      setTimeout(function() {
        d3.select('.anim')
    ...
    isHide(item) {
      if (item.ref === 'left') {
        ...
        this.leftStyle['width'] = '50px'
        this.rightStyle['width'] = 'calc(100% - 50px)'
        this.logoFlag = false
        ...
        this.leftStyle['width'] = '137px'
        this.rightStyle['width'] = 'calc(100% - 137px)'
        this.logoFlag = true
        this.allCategoryInform.forEach(val => {
    ...
    jsutTest(val) {
      if (val === 'download') {
        ...
        const className = document.getElementsByClassName('category-selected')[0].getElementsByTagName('i')[0].className
        const iconName = className.split(' ')[1]
        let index = 0
        // consoleconsole.log(this.allCategoryInform)
        for (; index < this.allCategoryInform.length; index++) {
    ...
    downloadSvg() { // 基本的图片下载还需改进!!
      this.dialogFormVisible = false
      this.svgDownloadList[this.selectedRaw].forEach(val => {
        const testDOM = document.querySelector(val)
        download.covertSVG2Image(testDOM, this.form.input, testDOM.width.baseVal.value, testDOM.height.baseVal.value, this.form.type)
      })
      this.form.input = ''
      this.form.type = ''
    },
    ...
    fontSize() {
      ...
      this.autoFontSize = parseInt(deviceWidth / 109)
      // console.log(deviceWidth, parseInt(deviceWidth / 109) + 'px')
